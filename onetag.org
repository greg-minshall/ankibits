...
#+BEGIN_SRC python :session ps :results silent
  ...
  def query_yes_no(question, default="yes", help=None, helparg1=None, helparg2=None):
      import sys
...
#+BEGIN_SRC python :results silent :noweb yes
  ...
  def dopcards(col, deckname):
      pcardsbyid(col, col.findCards("deck:%s" % deckname))
#+END_SRC
  ...
  def superset(col, deckname, tag):
      ...
      if ids:
          ...
          if query_yes_no("proceed with import?", default=None,
                          help=pcards, helparg1=col, helparg2=ids) == 'no':
              abort()
