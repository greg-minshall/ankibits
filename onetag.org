...
#+BEGIN_SRC python :noweb yes :tangle ankiimport :shebang "#!/usr/bin/env python"
  ...
  def superset(col, deckname, tag):
      ...
      if ids:
          if verbosity:
              ...
              print "will move %s cards into deck %s" % (len(ids), deckname)
              if query_yes_no("proceed with import?", default=None) == 'yes':
  ...
  def subset(col, deckname, tag):
      ...
      if ids:
          if verbosity:
              ...
              print "will delete %s cards from deck %s" % (len(ids), deckname)
              if query_yes_no("proceed with import?", default=None) == 'yes':
                  col.remCards(ids, notes=True) # not sure about notes=True,
                                                # but it makes sense for how we use it
  ...
  def doimport(col, deckname, ifilepath, dosubset, dosuperset):
