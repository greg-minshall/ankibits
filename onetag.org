...
#+BEGIN_SRC python :results silent :noweb yes
  ...
  def fcard(col, id, decknamep=False):
      ...
      if (verbosity):
          ...
          verbosenesses["interval=%d"] = str(card.ivl * 86400)
          verbosenesses["ease=%d"] = str(card.factor/10.0)
          verbosenesses["reviews=%d"] = str(card.reps)
          verbosenesses["lapses=%d"] = str(card.lapses)
